...
io_reg_t
directRead(volatile io_reg_t *base, io_reg_t pin)
{
    io_reg_t ret;
...
static inline __attribute__((always_inline)) void directModeInput(volatile io_reg_t *base, io_reg_t pin)
{
    if (SS_GPIO == GPIO_TYPE(pin))
    {
...
static inline __attribute__((always_inline)) void directModeOutput(volatile io_reg_t *base, io_reg_t pin)
{
    if (SS_GPIO == GPIO_TYPE(pin))
    {
        WRITE_ARC_REG(READ_ARC_REG(((io_reg_t)(base) + DIR_OFFSET_SS)) | (0x01 << GPIO_ID(pin)),
...
static inline __attribute__((always_inline)) void directWriteLow(volatile io_reg_t *base, io_reg_t pin)
{
    if (SS_GPIO == GPIO_TYPE(pin))
    {
...
static inline __attribute__((always_inline)) void directWriteHigh(volatile io_reg_t *base, io_reg_t pin)
{
    if (SS_GPIO == GPIO_TYPE(pin))
    {
        WRITE_ARC_REG(READ_ARC_REG(base) | (0x01 << GPIO_ID(pin)), base);
...
bool memcmp(const T1 *const source_A, const T1 *const source_B, const T2 bytes) // return true if string is different
{
    ...
    for (T2 counter = 0; counter < iterations; ++counter)
    {
        if (source_A[counter] != source_B[counter])
            return true;
    };
    return false;
}
